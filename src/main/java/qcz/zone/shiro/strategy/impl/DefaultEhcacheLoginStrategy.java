...
public class DefaultEhcacheLoginStrategy implements AbstractLoginStrategy {
    ...
    public DefaultEhcacheLoginStrategy(CacheManager cacheManager) {
        ehCacheShiroLock = new EhCacheShiroLock(cacheManager);
        if (null == ehCacheShiroLock)
            throw new RuntimeException("【DefaultEhcacheLoginStrategy】 EhCacheShiroLock is null");
    }
